...
| 🎨 | 多主题 | 8 套视觉主题，localStorage 记忆，首屏前注入防闪烁 |
...
- **代码高亮**：`pyHighlight()` 正则分词，输出 `tok-k`（关键字）/ `tok-s`（字符串）/ `tok-c`（注释）/ `tok-n`（数字）/ `tok-b`（内建函数）/ `tok-d`（装饰器）六类 token，与八套主题 CSS 变量联动。
...
- **极简主题**：致敬 suckless.org 的「极简」主题 —— 白底黑字、方正无圆角、无阴影、无毛玻璃，可用 `data-theme="suckless"` 单独启用；另有同风格的「极简暗色」`data-theme="suckless-dark"`（黑底白字，深浅互反）。
